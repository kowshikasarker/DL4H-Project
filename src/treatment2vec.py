...
#data_path = '/content/drive/My Drive/DL4H/DL4H_Project/Project/Paper46/Data'
...
class Symptom2Vec(nn.Module):
  def __init__(self, symptom_size, treatment_size, hidden_size=16):
    ...
    self.s2h = nn.Linear(symptom_size + hidden_size, 2*hidden_size, bias= True)
    self.h2o = nn.Linear(2*hidden_size,hidden_size, bias=True)
...
class Treatment2Vec(nn.Module):
  def __init__(self, symptom_size, treatment_size, hidden_size=16):
    ...
    self.hidden_size = hidden_size
    self.s2h = nn.Linear(treatment_size + hidden_size, 2*hidden_size, bias= True)
    self.h2o = nn.Linear(2*hidden_size,hidden_size, bias=True)


  def forward(self, symptom, treatment, hidden):
      combined_symptom = torch.cat((treatment, hidden), 1)
...
device = check_device()
pat2vec = Treatment2Vec(len(vocab_symptom), len(vocab_treatment) ,hidden_size= 16)
diag_pred = Diagnosis_Pred(hidden_size= 16, diagnosis_size=len(vocab_diagnosis)).to(device)
